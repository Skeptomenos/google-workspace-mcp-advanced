...
def test_resolve_auth_plan_prefers_callback_for_mapped_web_stdio(monkeypatch):
    monkeypatch.setenv("WORKSPACE_MCP_AUTH_FLOW", AUTH_FLOW_AUTO)
    monkeypatch.setattr("auth.google_auth.get_transport_mode", lambda: "stdio")
    monkeypatch.setattr(
    ...
    plan = _resolve_auth_plan("user@example.com")

    assert plan.selected_flow == AUTH_FLOW_CALLBACK
    assert plan.preferred_redirect_ports == (9876,)
    assert plan.allow_sequential_fallback is False


def test_resolve_auth_plan_prefers_device_for_installed_stdio(monkeypatch):
    monkeypatch.setenv("WORKSPACE_MCP_AUTH_FLOW", AUTH_FLOW_AUTO)
    monkeypatch.setattr("auth.google_auth.get_transport_mode", lambda: "stdio")
    monkeypatch.setattr(
        "auth.google_auth._resolve_oauth_client_selection",
    ...
    assert plan.selected_flow == AUTH_FLOW_DEVICE
    assert plan.preferred_redirect_ports == ()
    assert plan.allow_sequential_fallback is True


def test_resolve_auth_plan_requires_explicit_redirect_uri_for_streamable_http(monkeypatch):
    monkeypatch.setenv("WORKSPACE_MCP_AUTH_FLOW", AUTH_FLOW_AUTO)
    monkeypatch.setattr("auth.google_auth.get_transport_mode", lambda: "streamable-http")
    monkeypatch.setattr(
        "auth.google_auth._resolve_oauth_client_selection",
