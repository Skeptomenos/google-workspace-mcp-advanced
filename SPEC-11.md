...
**Issue:** #11  
**Status:** Phase 1 complete and verified; Phase 2 complete and verified; Phase 3 deferred follow-up  
**Author:** OpenCode  
**Last updated:** 2026-03-17  
**Supersedes:** the desktop-only implementation strategy in `issue-11.md`

## 0. Implementation Closeout (2026-03-17)

This section records the final implementation state and verification evidence for SPEC-11.
The earlier Phase 2 resume checkpoint is retained here as historical detail, but the spec is now
closed through Phase 2.
...
### Completed after starting Phase 2

- **Task 7 completed:**
  - `auth/auth_plan.py` now defines the frozen `ResolvedAuthPlan` dataclass.
  - Transport and flow fields are now constrained to `Literal["stdio", "streamable-http"]` and `Literal["device", "callback"]`.
  - Redirect-instance state remains out of the plan object.
  - Coverage added in `tests/unit/auth/test_auth_plan.py`.
- **Task 8 completed:**
  - `auth/flow_policy.py` now owns pure flow-selection rules via `resolve_auth_flow_plan(...)`.
  - `auth/google_auth.py` now composes `_resolve_auth_plan(...)` from resolved OAuth client data plus the flow policy seam.
  - `_get_effective_auth_flow_mode()` preserves backward-compatible `oauth_client.flow_preference` overrides before full plan resolution.
  - Coverage added in `tests/unit/auth/test_flow_policy.py` and extended in `tests/unit/auth/test_google_auth_flow_modes.py`.
- **Task 9 completed:**
  - `auth/redirect_policy.py` now owns redirect parsing and redirect-policy resolution.
  - `ResolvedRedirectPolicy` now carries callback allowance, preferred registered ports, sequential fallback policy, and explicit redirect requirements.
  - `auth/google_auth.py` now composes redirect policy into `_resolve_auth_plan(...)` instead of deriving redirect requirements from scattered helpers.
  - `auth/oauth_callback_server.py` now reuses redirect-policy URI helpers instead of rebuilding allowed localhost callback URIs inline.
  - Coverage added in `tests/unit/auth/test_redirect_policy.py` and extended in `tests/unit/auth/test_google_auth_flow_modes.py`.
- **Task 10 completed:**
  - `auth/challenge_store.py` now defines the frozen `AuthChallengeContext` dataclass with typed fields for `state`, `user_google_email`, `oauth_client_key`, `redirect_uri`, `code_verifier`, and `session_id`.
  - `challenge_context_to_dict()` and `challenge_context_from_dict()` provide roundtrip serialization to/from the raw dict format used by `OAuth21SessionStore`.
  - `core/server.py` now uses `challenge_context_from_dict()` in `_get_persisted_redirect_uri_for_state()` instead of raw dict `.get()` access.
  - `auth/google_auth.py` `handle_auth_callback()` now uses `challenge_context_from_dict()` to extract typed challenge fields instead of raw dict access.
  - Coverage added in `tests/unit/auth/test_challenge_store.py` (8 tests).
- **Task 11 completed:**
  - `tests/integration/auth/test_auth_matrix.py` now provides a 23-test regression matrix.
  - Covers: web vs installed, stdio vs streamable-http, callback vs device, flow preference overrides,
    mapped-only vs legacy env fallback, missing client_type repair, occupied callback ports,
    concurrent callback reuse, wrong-account detection, pending state restart roundtrip, and
    legacy `expected_user_email` key compatibility.
- **Task 12 completed:**
  - `auth/ARCHITECTURE.md` updated to reflect Phase 2 extracted modules, dependency graph, policy composition,
    and auth topology support matrix.
  - `docs/setup/AUTHENTICATION_MODEL.md` updated with support matrix and policy module references.
...
# focused Task 9 redirect slice: 10 passed
# focused Task 10 challenge store slice: 44 passed
# Task 11 auth matrix: 23 passed
# auth unit slice: 142 passed
 # full pytest: 794 passed, 3 skipped
```

### Phase 2 closeout

- Phase 1 is complete and verified.
- All Phase 2 tasks (7-12) are complete and verified.
- Phase 2 exit criteria are met: flow and redirect behavior are controlled by small pure modules,
  challenge persistence is explicit and typed, the auth matrix exists and passes, and architecture
  docs reflect the extracted module boundaries.
- Any further auth simplification belongs to deferred Phase 3 work, not this spec closeout.

### Deferred follow-up note

- `issue-11.md` remains superseded by this spec and is intentionally left outside the shipped spec path.
- Any future Phase 3 work should start from the already-green Phase 2 auth matrix rather than reopening
  Phase 1 or Phase 2 behavior.
...
- `streamable-http` / remote HTTP deployments
...
class ResolvedAuthPlan:
    ...
    transport_mode: Literal["stdio", "streamable-http"]
...
- Do not start a local callback server from non-stdio / HTTP code paths as an implicit fallback.
...
## 7. Phase 2 Closeout and Phase 3 Follow-up

All Phase 2 gaps originally tracked by this section are resolved:

1. Redirect parsing and redirect-policy resolution now live in `auth/redirect_policy.py`.
2. Callback-server reuse now consumes shared redirect-policy helpers in `auth/oauth_callback_server.py`.
3. `ResolvedAuthPlan.requires_explicit_redirect_uri` is owned by redirect policy instead of being
   inferred inside `auth/flow_policy.py`.
4. OAuth challenge persistence now has a typed facade in `auth/challenge_store.py` instead of raw dict
   handling spread across multiple call sites.
5. The repository now includes the internal auth regression matrix at
   `tests/integration/auth/test_auth_matrix.py`.

Phase 3 remains deferred follow-up work and is intentionally out of scope for SPEC-11 closeout:

1. Split stdio/local auth orchestration from provider / HTTP orchestration into dedicated modules
   (planned: `auth/stdio_auth.py` and `auth/http_auth.py`).
2. Remove remaining error-driven or ambiguous runtime fallback paths so `auto` becomes fully
   capability-based.
3. Reduce `auth/google_auth.py` to a thin compatibility facade and delete dead compatibility branches
   once the topology split lands.

See `docs/plans/2026-03-17-phase-3-oauth-topology-refactor.md` for the dedicated deferred
Phase 3 plan.
...
[Resolve callback policy]
   ...
   |- streamable-http / remote callback path -> explicit configured redirect URI behavior
              |
              v
...
- If stored OAuth-state metadata for manual completion cannot be loaded:
  - `authorization_code + state` must fail cleanly
  - explicit `callback_url` completion may fall back to the callback URL's own base URI, but never to
    current runtime redirect config
...
**Status:** Complete and released in `v1.0.10`.
...
- Cover at least:
  ...
  - `streamable-http` transport -> callback by default
  - no runtime-error-driven selection in the final Phase 2 behavior
...
**Implementation:**
- Move `_extract_ports_from_redirect_uris()` and `_resolve_callback_port_policy()` out of
  `auth/google_auth.py` into a pure redirect-policy module.
...
- `_resolve_auth_plan()` should compose `resolve_redirect_policy()` with `resolve_auth_flow_plan()`
  instead of deriving redirect requirements indirectly.
- `ResolvedAuthPlan.requires_explicit_redirect_uri` must become redirect-policy-owned state instead of
  being inferred inside `auth/flow_policy.py`.
- If a tiny helper for allowed localhost redirect URIs reduces duplication, keep it in the redirect
  policy seam rather than rebuilding that logic in multiple call sites.
...
- Add a short support matrix explaining which auth topology is supported in `stdio` vs `streamable-http`
  mode.
...
- the Phase 3 stdio/local vs provider/HTTP topology split is now tracked in
  `docs/plans/2026-03-17-phase-3-oauth-topology-refactor.md`
