...
**Status:** Phase 1 and Phase 2 are implemented and released in `v1.0.10`.  
**Follow-up:** Deferred structural work now lives in `docs/plans/2026-03-17-phase-3-oauth-topology-refactor.md`.
...
Phase 1 and Phase 2 from this composite plan are now implemented. The remaining structural follow-up
has been split into a dedicated document:

- `docs/plans/2026-03-17-phase-3-oauth-topology-refactor.md`

That dedicated plan now owns:

1. stdio/local vs provider/HTTP topology separation
2. removal of remaining error-driven steady-state fallback
3. shrinking `auth/google_auth.py` into a thin facade

This composite document remains as the historical source for the original three-phase decomposition and
the implemented Phase 1/Phase 2 rationale.
