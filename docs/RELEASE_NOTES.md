...
## 2026-03-17 - Phase 2 Auth Policy Extraction Closeout + Release v1.0.10

### Added
- Pure typed auth policy seams for deterministic auth planning:
  - `auth/auth_plan.py`
  - `auth/flow_policy.py`
  - `auth/redirect_policy.py`
  - `auth/challenge_store.py`
- Internal auth regression matrix coverage in `tests/integration/auth/test_auth_matrix.py`.

### Changed
- `auth/google_auth.py` now composes extracted flow and redirect policy modules instead of owning the
  full policy decision tree inline.
- Callback completion and manual completion now share the typed `AuthChallengeContext` facade over the
  persisted OAuth state store.
- Auth architecture and operator docs now describe the extracted policy seams and explicitly treat
  Phase 3 topology simplification as deferred follow-up work.
- Version bump to `1.0.10` in release artifacts:
  - `pyproject.toml`
  - `uv.lock`
- Updated pinned `uvx` examples in README and setup/distribution docs to `1.0.10`.
