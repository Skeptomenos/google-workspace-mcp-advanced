# User Documentation Index

Audience: users and operators running this MCP in clients such as OpenCode, Claude Code, and Gemini CLI.

## Start Here

1. Install `uv`.
2. Configure your MCP client.
3. Authenticate your Google account.
4. Use a pinned `uvx` version for stable team rollouts.

## Guides

- Setup and client configuration: [MCP Client Setup Guide](setup/MCP_CLIENT_SETUP_GUIDE.md)
- Authentication model and runtime behavior: [Authentication Model](setup/AUTHENTICATION_MODEL.md)
- Single-MCP multi-client auth routing: [Multi-Client Auth Setup](setup/MULTI_CLIENT_AUTH_SETUP.md)
- Apps Script setup and runtime limitations: [Apps Script Setup](setup/APPS_SCRIPT_SETUP.md)
- Apps Script capabilities and workflows: [Apps Script User Guide](setup/APPS_SCRIPT_USER_GUIDE.md)
- Apps Script + clasp workflows: [Apps Script + clasp Guide](setup/APPS_SCRIPT_CLASP_GUIDE.md)
- Migration from legacy naming/path: [Migration Guide](setup/MIGRATING_FROM_GWS_MCP_ADVANCED.md)
- Claude Code setup: [Claude Code MCP Setup](setup/CLAUDE_CODE_MCP_SETUP.md)
- Cursor setup: [Cursor MCP Setup](setup/CURSOR_MCP_SETUP.md)
- OpenCode setup: [OpenCode MCP Setup](setup/OPENCODE_MCP_SETUP.md)
- Gemini CLI setup: [Gemini CLI MCP Setup](setup/GEMINI_CLI_MCP_SETUP.md)
- Live test cadence automation: [Cadence Workflow User Guide](setup/LIVE_CADENCE_WORKFLOW.md)
- Release/versioning/rollback: [Distribution and Release Guide](DISTRIBUTION_RELEASE.md)
- Release notes: [Release Notes](RELEASE_NOTES.md)
- Product overview and upstream comparison: [Comparison](COMPARISON.md)

## Notes

- Recommended runtime path is `uvx`.
- Contributor planning and test execution docs live under `agent-docs/`.
