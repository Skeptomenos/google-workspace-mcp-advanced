...
| Transport | Primary Client Type | Auth Flow (auto) | Sequential Callback Fallback | Explicit Redirect URI |
|---|---|---|---|---|
| `stdio` | `installed` | device | Allowed | No |
| `stdio` | `web` (mapped) | callback | Disabled (registered ports only) | Yes |
| `stdio` | legacy env | device | Allowed | No |
| `streamable-http` | any | callback | Disabled | Yes |

Policy decisions are owned by pure modules:
- **Flow selection:** `auth/flow_policy.py` (`resolve_auth_flow_plan`)
- **Redirect rules:** `auth/redirect_policy.py` (`resolve_redirect_policy`)
- **Challenge persistence:** `auth/challenge_store.py` (`AuthChallengeContext`)
- **Composition:** `auth/google_auth.py` (`_resolve_auth_plan`) composes both policy seams
