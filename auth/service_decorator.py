...
def _detect_oauth_version(authenticated_user: str | None, mcp_session_id: str | None, tool_name: str) -> bool:
    """
...
async def get_authenticated_google_service_oauth21(
    ...
    if provider and access_token:
        ...
        if not resolved_email:
            raise GoogleAuthenticationError("Authenticated user email could not be determined from access token.")

        if auth_token_email and token_email and token_email != auth_token_email:
            raise GoogleAuthenticationError("Access token email does not match authenticated session context.")
        ...
        if not credentials:
            raise GoogleAuthenticationError("Unable to build Google credentials from authenticated access token.")
