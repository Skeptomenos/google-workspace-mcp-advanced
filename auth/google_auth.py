...
from auth.auth_plan import ResolvedAuthPlan, TransportMode
...
def _get_effective_auth_flow_mode(
    ...
    try:
        ...
        plan = _resolve_auth_plan(user_google_email, override_client_key)
        return plan.selected_flow
...
def _resolve_auth_plan(
    user_google_email: str,
    override_client_key: str | None = None,
) -> ResolvedAuthPlan:
    """Resolve the typed auth decision for one auth attempt."""
    oauth_client = _resolve_oauth_client_selection(user_google_email, override_client_key)
    transport_mode = cast(TransportMode, get_transport_mode())
    redirect_policy = resolve_redirect_policy(oauth_client=oauth_client, transport_mode=transport_mode)
    return resolve_auth_flow_plan(
        requested_auth_mode=_get_auth_flow_mode(),
        transport_mode=transport_mode,
        user_google_email=user_google_email,
        oauth_client=oauth_client,
        preferred_redirect_ports=redirect_policy.preferred_redirect_ports,
        allow_sequential_fallback=redirect_policy.allow_sequential_fallback,
        requires_explicit_redirect_uri=redirect_policy.requires_explicit_redirect_uri,
    )
...
async def _start_callback_auth_challenge(
    ...
    """
    plan = _resolve_auth_plan(user_google_email, override_client_key)
    oauth_client = plan.oauth_client
    assert oauth_client is not None
    preferred_ports = list(plan.preferred_redirect_ports)
    if preferred_ports:
        logger.info(
    ...
    oauth_redirect_uri = resolve_oauth_redirect_uri_for_auth_flow(
        ...
        allow_sequential_fallback=plan.allow_sequential_fallback,
    )
...
async def handle_auth_callback(
    ...
    try:
        ...
        challenge = challenge_context_from_dict({**state_info, "state": state})
        state_oauth_client_key = challenge.oauth_client_key
        expected_user_email = challenge.user_google_email
        code_verifier = challenge.code_verifier
        redirect_uri_for_flow = challenge.redirect_uri or redirect_uri
        logger.debug(
            "Validated OAuth callback state %s for session %s",
            (state[:8] if state else "<missing>"),
            challenge.session_id or "<unknown>",
        )
